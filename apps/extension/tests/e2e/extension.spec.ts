...
test.describe('Theme', () => {
  test('app container has styled background', async () => {
    const bgColor = await page.locator('.app-container').evaluate((el) => window.getComputedStyle(el).backgroundColor);
    expect(bgColor).toBeTruthy();
    expect(bgColor).not.toBe('');
  });

  test('footer has a border-top style', async () => {
    const borderTop = await page.locator('.footer').evaluate((el) => window.getComputedStyle(el).borderTop);
