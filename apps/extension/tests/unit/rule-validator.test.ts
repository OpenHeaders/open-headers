...
const BASE64_VALUE = 'SGVsbG8gT3BlbkhlYWRlcnMhIFRoaXMgaXMgYSBiYXNlNjQgZW5jb2RlZCB2YWx1ZSBmb3IgdGVzdGluZy4=';
...
describe('validateHeaderValue', () => {
  ...
  it('returns invalid result for value with null bytes', () => {
    const result: HeaderValueValidation = validateHeaderValue('Bearer\0injected', 'Authorization');
    expect(result.valid).toBe(false);
    expect(result.message).toContain('null');
  });

  it('returns invalid result for value with CRLF injection', () => {
    const result: HeaderValueValidation = validateHeaderValue('Bearer token\r\nSet-Cookie: evil=1', 'Authorization');
    expect(result.valid).toBe(false);
  });

  it('returns invalid result for value with control chars', () => {
    const result: HeaderValueValidation = validateHeaderValue('\x01\x02\x03', 'X-Custom');
    expect(result.valid).toBe(false);
  });

  it('returns invalid result for oversized value', () => {
    const result: HeaderValueValidation = validateHeaderValue('B'.repeat(8193), 'X-Huge');
    expect(result.valid).toBe(false);
