...
describe('previewAuthContributions', () => {
  it('contributes nothing for none / inherit', () => {
    for (const type of ['none', 'inherit'] as const) {
      const out = previewAuthContributions({ type }, t);
  ...
  it('basic + bearer add an Authorization header with their scheme', () => {
    const basic = previewAuthContributions({ type: 'basic', username: 'u', password: 'p' }, t);
    ...
    const bearer = previewAuthContributions({ type: 'bearer', token: 't' }, t);
  ...
  it('never leaks the resolved credential into the preview value', () => {
    const basic = previewAuthContributions({ type: 'basic', username: 'admin', password: 'hunter2' }, t);
    expect(basic.headers[0].value).not.toContain('hunter2');
    const bearer = previewAuthContributions({ type: 'bearer', token: 'super-secret-token' }, t);
  ...
  it('api-key lands in the header or the query depending on `in`', () => {
    const inHeader = previewAuthContributions({ type: 'api-key', key: 'X-Api-Key', value: 's', in: 'header' }, t);
    expect(inHeader.headers).toHaveLength(1);
    expect(inHeader.headers[0].key).toBe('X-Api-Key');
    expect(inHeader.params).toEqual([]);

    const inQuery = previewAuthContributions({ type: 'api-key', key: 'api_key', value: 's', in: 'query' }, t);
  ...
  it('api-key with no key contributes nothing', () => {
    const out = previewAuthContributions({ type: 'api-key', key: '   ', value: 's', in: 'header' }, t);
    expect(out.headers).toEqual([]);
    expect(out.params).toEqual([]);
  });
  ...
  it('oauth2 sends a Bearer header by default, or access_token on the URL when sendAs=query', () => {
    ...
    const header = previewAuthContributions(base satisfies AuthConfig, t);
    expect(header.headers[0].key).toBe('Authorization');
    expect(header.headers[0].value).toMatch(/^Bearer /);

    const query = previewAuthContributions({ ...base, sendAs: 'query' }, t);
