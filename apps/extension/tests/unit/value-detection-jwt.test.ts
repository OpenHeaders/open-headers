/**
 * JWT codec + value-type detection — the pure half of the JWT value
 * editor. Ported from the legacy desktop jwtUtils suite (re-signing
 * excluded — the v5 editor carries the original signature over) plus
 * the detector-registry contract: bare tokens and `Bearer `-prefixed
 * tokens detect as JWT with the prefix preserved for write-back.
 */
...
describe('jwt codec', () => {
  ...
  describe('validateJSON', () => {
    ...
    it('parses enterprise config JSON', () => {
      ...
      const parsed = validateJSON(config) as {
        sources: unknown[];
        environments: { Production: { API_KEY: string } };
      };
...
describe('detectValueType', () => {
  it('detects a bare JWT', () => {
    const token = buildJWT(ENTERPRISE_HEADER, ENTERPRISE_PAYLOAD);
    expect(detectValueType(token)).toEqual({ type: 'jwt', token, prefix: '' });
  });

  it('detects a Bearer-prefixed JWT, preserving the prefix', () => {
    const token = buildJWT({ alg: 'HS256' }, { sub: 'user@openheaders.io' });
    expect(detectValueType(`Bearer ${token}`)).toEqual({ type: 'jwt', token, prefix: 'Bearer ' });
  });
  ...
  it('returns null for a non-JWT value', () => {
    expect(detectValueType('ohk_live_4eC39HqLyjWDarjtT1zdp7dc')).toBeNull();
  });

  it('returns null for a Bearer value that is not a JWT', () => {
    expect(detectValueType('Bearer ohk_live_4eC39HqLyjWDarjtT1zdp7dc')).toBeNull();
  });

  it('returns null for empty and undefined values', () => {
    expect(detectValueType('')).toBeNull();
    expect(detectValueType(undefined)).toBeNull();
  });

  it('returns null for a template reference', () => {
    expect(detectValueType('Bearer {{env.TOKEN}}')).toBeNull();
  });
});
