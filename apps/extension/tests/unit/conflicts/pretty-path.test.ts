...
describe('prettyRulePath', () => {
  it('renders top-level name + scalar fields', () => {
    expect(prettyRulePath(t, RULE, 'name')).toBe('Name');
    const redirect = { ...RULE, type: 'redirect', action: { redirectTo: '' } } as unknown as Rule;
    expect(prettyRulePath(t, redirect, 'action.redirectTo')).toBe('Redirect URL');
    const delay = { ...RULE, type: 'delay', action: { delayMs: 100 } } as unknown as Rule;
    expect(prettyRulePath(t, delay, 'action.delayMs')).toBe('Delay (ms)');
  });

  it('substitutes header name when the row resolves', () => {
    expect(prettyRulePath(t, RULE, 'action.requestHeaders.aaaaaaaa.value')).toBe('Request header X-Auth (value)');
    expect(prettyRulePath(t, RULE, 'action.responseHeaders.cccccccc.headerName')).toBe(
      'Response header X-Cache (name)',
    );
  });

  it('falls back gracefully when the row uid is unknown', () => {
    expect(prettyRulePath(t, RULE, 'action.requestHeaders.deadbeef.value')).toBe('Request header (value)');
  });
  ...
  it('renders response + query-param + condition shapes', () => {
    ...
    expect(prettyRulePath(t, response, 'action.responseHeaders.X-Foo.value')).toBe('Response header X-Foo (value)');
    ...
    expect(prettyRulePath(t, qp, 'action.params.bbbbbbbb.param')).toBe('Query param foo (name)');

    expect(prettyRulePath(t, RULE, 'conditions.aaaaaaaa.values')).toBe('Condition values');
  });

  it('returns the raw path when the structure is not recognized', () => {
    expect(prettyRulePath(t, RULE, 'totally.unknown.path')).toBe('totally.unknown.path');
  });
});
