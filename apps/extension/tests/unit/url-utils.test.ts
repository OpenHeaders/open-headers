...
describe('normalizeUrlForTracking', () => {
  ...
  it('handles URL with port, fragment, and query together', () => {
    const result = normalizeUrlForTracking('https://api.partner-service.io:9200/search?q=headers#results');
...
describe('doesUrlMatchPattern', () => {
  ...
  it('matches wildcard subdomain pattern *.openheaders.io', () => {
    expect(doesUrlMatchPattern('https://api.openheaders.io/v2/config', '*.openheaders.io')).toBe(true);
    expect(doesUrlMatchPattern('https://dashboard.openheaders.io/settings', '*.openheaders.io')).toBe(true);
  });
  ...
  it('does not match bare domain against wildcard subdomain pattern', () => {
    ...
    expect(doesUrlMatchPattern('https://openheaders.io/', '*.openheaders.io')).toBe(false);
  });

  it('matches exact domain pattern', () => {
    expect(doesUrlMatchPattern('https://api.openheaders.io/v2/config', 'api.openheaders.io')).toBe(true);
  });

  it('does not match different domain', () => {
    expect(doesUrlMatchPattern('https://api.openheaders.io/v2/config', 'dashboard.openheaders.io')).toBe(false);
  });
  ...
  it('matches IPv6 address pattern in brackets', () => {
    expect(doesUrlMatchPattern('http://[::1]:3000/api', '[::1]:3000')).toBe(true);
  });

  it('normalizes default port 443 in pattern', () => {
    // Pattern with :443 should match URL without explicit port (https default)
    expect(doesUrlMatchPattern('https://api.openheaders.io/v2/config', 'api.openheaders.io:443')).toBe(true);
  });

  it('normalizes default port 80 in pattern', () => {
    expect(doesUrlMatchPattern('http://api.openheaders.io/v2/config', 'api.openheaders.io:80')).toBe(true);
  });

  it('matches pattern with explicit path', () => {
    expect(doesUrlMatchPattern('https://api.openheaders.io/v2/config', 'api.openheaders.io/v2/*')).toBe(true);
  });

  it('does not match pattern with non-matching path', () => {
    expect(doesUrlMatchPattern('https://api.openheaders.io/v3/config', 'api.openheaders.io/v2/*')).toBe(false);
  });
  ...
  it('handles pattern with protocol prefix', () => {
    expect(doesUrlMatchPattern('https://api.openheaders.io/v2/config', 'https://api.openheaders.io/*')).toBe(true);
  });
...
describe('pattern cache', () => {
  ...
  it('precompileAllPatterns compiles multiple patterns', () => {
    ...
    expect(doesUrlMatchPattern('https://api.openheaders.io/v2/config', 'api.openheaders.io')).toBe(true);
    expect(doesUrlMatchPattern('https://staging.partner-service.io/health', '*.partner-service.io')).toBe(true);
    expect(doesUrlMatchPattern('http://localhost:3000/api', 'localhost:3000')).toBe(true);
  });
  ...
  it('doesUrlMatchPattern works without pre-compilation (lazy compile)', () => {
    ...
    expect(doesUrlMatchPattern('https://example.com/page', 'example.com')).toBe(true);
  });
});
