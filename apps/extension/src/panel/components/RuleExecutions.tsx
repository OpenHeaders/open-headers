  ...
  const groups = useMemo<RuleGroup[]>(() => {
    ...
    for (const e of entries) {
      for (const f of e.fires) {
        ...
        if (!g) {
          g = { ruleUid: f.ruleUid, totalHits: 0, authoritativeHits: 0, attachedHits: [], danglingHits: [] };
          byRule.set(f.ruleUid, g);
        }
        g.totalHits++;
        if (f.authoritative) g.authoritativeHits++;
    ...
    for (const d of danglingFires) {
      ...
      if (!g) {
        g = { ruleUid: d.ruleUid, totalHits: 0, authoritativeHits: 0, attachedHits: [], danglingHits: [] };
        byRule.set(d.ruleUid, g);
      }
      g.totalHits++;
      if (d.authoritative) g.authoritativeHits++;
  ...
  return (
    <div className="dt-panel">
      ...
      <div className="dt-executions-hint" style={{ marginBottom: 6 }}>
        ...
        fires are rule matches on requests DevTools didn&apos;t capture.
      </div>
      {groups.map((group) => (
        <details key={group.ruleUid} className="dt-exec-group" open>
          <summary>
            ...
            {group.authoritativeHits > 0 && (
              <span className="dt-exec-badge dt-exec-badge--auth">{group.authoritativeHits} authoritative</span>
            )}
