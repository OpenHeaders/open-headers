 ...
 * iframe of the workbench page, driven by core's broker over the
 * shared sandbox iframe transport (`@openheaders/oracle-host-browser`
 * — the served web tab mounts its own sandbox page through the same
 * one).
 *
...
import { createScriptBroker, type ScriptBroker } from '@openheaders/core/scripts/broker';
...
function pageBroker(): ScriptBroker {
  if (broker === null) {
    broker = createScriptBroker({
      createTransport: createIframeSandboxTransport({ src: getBrowserAPI().runtime.getURL(SANDBOX_PAGE) }),
