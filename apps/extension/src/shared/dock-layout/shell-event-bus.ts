 ...
 * for the tool-window shell.
 *
 * Attaches exactly one listener per event type on the shell root (click in
 * capture phase, focusin, focusout) and one keydown listener on the window.
 * Consumers subscribe via React hooks; their callbacks run in subscription
 * order. If you're tempted to call `setState` inside one of these
 * subscribers for ambient UI state (focus accents, hover tracking, etc.),
 * write to an external store instead — re-rendering the form tree from a
 * click-capture handler is what breaks antd radios.
 */
