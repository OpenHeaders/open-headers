      ...
      for (const { from, to } of copies) {
        ...
        if (fs.existsSync(src)) {
          ...
          if (to === 'manifest.json') {
            ...
            // `chrome://extensions` so `26.6.0-beta.1` is not silently
            // displayed as `26.6.0.1`.
...
export default defineConfig({
  ...
  // __APP_VERSION__ uses the numeric manifest-style version (e.g. 26.6.0.1 instead of 26.6.0-beta.1)
