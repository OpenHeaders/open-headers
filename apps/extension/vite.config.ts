      ...
      for (const { from, to } of copies) {
        ...
        if (fs.existsSync(src)) {
          ...
          if (to === 'manifest.json') {
            ...
            // `chrome://extensions` so `2026.6.2-beta.1` is not silently
            // displayed as `2026.6.2.1`.
...
export default defineConfig({
  ...
  // __APP_VERSION__ uses the numeric manifest-style version (e.g. 2026.6.2.1 instead of 2026.6.2-beta.1)
