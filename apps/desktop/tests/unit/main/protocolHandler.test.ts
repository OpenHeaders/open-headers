...
function makeCompressedUrl(payload: Record<string, unknown>, compression: 'gzip' | 'deflate' = 'gzip'): string {
  ...
  const compressed = compression === 'deflate' ? zlib.deflateSync(json) : zlib.gzipSync(json, { level: 9 });
...
describe('ProtocolHandler', () => {
  ...
  describe('setMainWindow', () => {
    it('stores window reference', () => {
      const win = { webContents: { send: () => {}, isLoading: () => false } } as unknown as Parameters<
        typeof handler.setMainWindow
      >[0];
  ...
  describe('processPendingInvites (renderer-ready flow)', () => {
    ...
    it('shows and focuses window when processing pending environment import', () => {
      ...
      expect(showCalls).toContain('show');
      expect(win.webContents.send).toHaveBeenCalledWith(
        'process-environment-config-import',
        expect.objectContaining({
