...
function makeMockWindow(
  overrides: { isDestroyed?: () => boolean; send?: (...args: unknown[]) => void } = {},
): BrowserWindow {
  return {
...
describe('CliSetupHandler', () => {
  ...
  describe('_normalizeAuthData()', () => {
    ...
    it('normalizes token auth with GitHub PAT', () => {
      expect(
        handler._normalizeAuthData('token', {
    ...
    it('normalizes ssh-key auth with key and passphrase', () => {
      ...
      expect(
        handler._normalizeAuthData('ssh-key', {
          sshKey,
    ...
    it('normalizes basic auth with special characters in credentials', () => {
      expect(
        handler._normalizeAuthData('basic', {
  ...
  describe('_generateUniqueWorkspaceName()', () => {
    it('returns base name when no conflicts exist', async () => {
      ...
      expect(await handler._generateUniqueWorkspaceName('OpenHeaders — Production Config', mockService)).toBe(
    ...
    it('appends (2) when base name already exists', async () => {
      ...
      expect(await handler._generateUniqueWorkspaceName('OpenHeaders — Production Config', mockService)).toBe(
        'OpenHeaders — Production Config (2)',
      );
    });
    ...
    it('increments counter past all existing conflicts', async () => {
      const mockService = {
        getSettings: async () => ({
          workspaces: [{ name: 'Platform Team' }, { name: 'Platform Team (2)' }, { name: 'Platform Team (3)' }],
        }),
      };
      expect(await handler._generateUniqueWorkspaceName('Platform Team', mockService)).toBe('Platform Team (4)');
    });

    it('returns base name when workspaces array is undefined', async () => {
      const mockService = { getSettings: async () => ({}) };
      expect(await handler._generateUniqueWorkspaceName('New Workspace', mockService)).toBe('New Workspace');
    });
    ...
    it('handles workspace names with unicode characters', async () => {
      ...
      expect(await handler._generateUniqueWorkspaceName('ÖpenHeaders — Ünified Cönfig', mockService)).toBe(
        'ÖpenHeaders — Ünified Cönfig (2)',
      );
    });
  });
  ...
  describe('_notifyRenderer()', () => {
    ...
    it('sends channel and data to window when available', () => {
      ...
      handler.mainWindow = makeMockWindow({
        send: (channel: unknown, data: unknown) => {
          sent = { channel: channel as string, data: data as NotifyData };
        },
      });
    ...
    it('sends environment structure change events', () => {
      ...
      handler.mainWindow = makeMockWindow({
        send: (channel: unknown) => {
          sentChannel = channel as string;
        },
      });
  ...
  describe('joinWorkspace()', () => {
    ...
    function setupMocks(
      overrides: { connectionSuccess?: boolean; syncSuccess?: boolean; syncData?: typeof mockSyncData | null } = {},
    ) {
      const { connectionSuccess = true, syncSuccess = true, syncData = mockSyncData } = overrides;

      mockTestConnection.mockResolvedValue({
        success: connectionSuccess,
        error: connectionSuccess ? undefined : 'Connection refused',
      });
      mockSyncWorkspace.mockResolvedValue({
        success: syncSuccess,
        data: syncData,
        error: syncSuccess ? undefined : 'Sync failed',
      });
