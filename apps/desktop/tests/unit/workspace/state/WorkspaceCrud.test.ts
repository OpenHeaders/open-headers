...
function createCtx(overrides: Partial<StateContext> = {}): StateContext {
  return {
    ...
    dirty: { sources: false, rules: false, proxyRules: false, collections: false, folders: false, workspaces: false, environments: false },
...
describe('createWorkspace', () => {
  ...
  it('rejects duplicate workspace ID', async () => {
    const ctx = createCtx();
    await expect(
      createWorkspace(ctx, { id: 'default-personal', name: 'Dup', type: 'personal' }, vi.fn()),
    ).rejects.toThrow('already exists');
  });

  it('rejects empty workspace name', async () => {
    const ctx = createCtx();
    await expect(createWorkspace(ctx, { id: 'ws-x', name: '', type: 'personal' }, vi.fn())).rejects.toThrow(
  ...
  it('rejects name over 100 chars', async () => {
    const ctx = createCtx();
    await expect(
      createWorkspace(ctx, { id: 'ws-x', name: 'A'.repeat(101), type: 'personal' }, vi.fn()),
  ...
  it('rejects invalid workspace type', async () => {
    const ctx = createCtx();
    await expect(
      createWorkspace(ctx, { id: 'ws-x', name: 'X', type: 'invalid' as 'personal' }, vi.fn()),
    ).rejects.toThrow('Invalid workspace type');
  });

  it('rejects invalid ID characters', async () => {
    const ctx = createCtx();
    await expect(createWorkspace(ctx, { id: 'ws x!', name: 'X', type: 'personal' }, vi.fn())).rejects.toThrow(
  ...
  it('requires gitUrl for git workspaces', async () => {
    const ctx = createCtx();
    await expect(createWorkspace(ctx, { id: 'ws-git', name: 'Git WS', type: 'git' }, vi.fn())).rejects.toThrow(
      'gitUrl',
    );
  });
...
describe('updateWorkspace', () => {
  ...
  it('throws for non-existent workspace', async () => {
    const ctx = createCtx();
    await expect(updateWorkspace(ctx, 'nonexistent', { name: 'X' })).rejects.toThrow('not found');
  });

  it('validates name over 100 chars', async () => {
    const ctx = createCtx();
    await expect(updateWorkspace(ctx, 'default-personal', { name: 'A'.repeat(101) })).rejects.toThrow(
      'Workspace name must be',
    );
  });
...
describe('deleteWorkspace', () => {
  ...
  it('refuses to delete default workspace', async () => {
    const ctx = createCtx();
    await expect(deleteWorkspace(ctx, 'default-personal', vi.fn())).rejects.toThrow('Cannot delete default');
  });
