 ...
 * flip rebinds in place — closing the old server and emitting a fresh
 * binding→bound for the new host.
 *
...
const startMock = vi.fn<(opts: { host: string }) => Promise<FakeServer>>();

vi.mock('@openheaders/oracle-host-node/host-runtime/ws-server', () => ({
  startOracleWsServer: (opts: { host: string }) => startMock(opts),
}));
...
function makeServer(host: string): FakeServer {
  return { host, close: vi.fn(async () => undefined) };
}
...
beforeEach(() => {
  ...
  settingsListener = null;
  startMock.mockReset();
  startMock.mockImplementation(async ({ host }) => makeServer(host));
});
...
describe('daemon-bind-supervisor — bind lifecycle', () => {
  it('emits binding→bound and hands over the server on a healthy boot', async () => {
    ...
    expect(states).toEqual([
      { kind: 'binding', host: '127.0.0.1' },
      { kind: 'bound', host: '127.0.0.1' },
    ]);
    expect(servers).toHaveLength(1);
    expect(servers[0]).toMatchObject({ host: '127.0.0.1' });
  ...
  it('honours an initial 0.0.0.0 setting', async () => {
    ...
    expect(states).toEqual([
      { kind: 'binding', host: '0.0.0.0' },
      { kind: 'bound', host: '0.0.0.0' },
    ]);
  ...
  it('emits binding→failed and leaves no server when the bind throws', async () => {
    ...
    expect(states).toEqual([
      { kind: 'binding', host: '127.0.0.1' },
      { kind: 'failed', host: '127.0.0.1', error: cause },
    ]);
  ...
  it('rebinds in place when backend.bindAddress flips, closing the old server', async () => {
    ...
    expect(firstServer).toMatchObject({ host: '127.0.0.1' });
  ...
  it('ignores a setting change that does not move the bind address', async () => {
    const states: DaemonBindState[] = [];
    ...
    settingsListener?.({ 'backend.bindAddress': '127.0.0.1' });
    await flush();
