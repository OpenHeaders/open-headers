...
async function seedBackend(seed: { enabled: boolean }): Promise<void> {
  ...
  await page.evaluate(
    async ({ backendUrl, authToken, enabled }) => {
      ...
      const record = {
        id: 'agent-traffic-tools-e2e-backend',
        label: 'agent-traffic tools e2e desktop',
        url: backendUrl,
        authToken,
        autoConnect: true,
        enabled,
...
async function seedBackendRetrying(seed: { enabled: boolean }): Promise<void> {
  ...
  for (let attempt = 0; attempt < 8; attempt++) {
    try {
      ...
      seedError = err;
      console.log(`[agent-traffic-tools setup] seed attempt ${attempt} failed: ${String(err).split('\n')[0]}`);
