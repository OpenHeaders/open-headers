...
export async function deleteWorkspace(
  ...
  await fs.promises
    ...
    .catch((e) => log.warn('Failed to delete workspace dir:', errorMessage(e)));
  await ctx.saveWorkspacesConfig();
...
export async function syncWorkspace(
  ctx: StateContext,
  workspaceId: string,
  _updateSyncStatus: (id: string, status: Record<string, unknown>) => void,
  ...
  return (
    ctx.syncScheduler as WorkspaceSyncSchedulerLike & {
      manualSync(id: string): Promise<{ success: boolean; error?: string }>;
    }
  ).manualSync(workspaceId);
}

export async function copyWorkspaceData(
  ctx: StateContext,
