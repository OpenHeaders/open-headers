...
class RecordingHandlers {
  ...
  async handleSaveRecording(_: IpcInvokeEvent, recordData: RecordingData) {
    try {
      ...
      const recordId =
        recordData.record.metadata.recordId || `record-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
  ...
  async handleSaveUploadedRecording(_: IpcInvokeEvent, recordData: RecordingData) {
    try {
      ...
      const recordId =
        recordData.record?.metadata?.recordId || `upload-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
  ...
  async handleUpdateRecordingMetadata(
    _: IpcInvokeEvent,
    { recordId, updates }: { recordId: string; updates: Partial<WorkflowRecordingFileMetadata> },
  ) {
    try {
      const recordingsPath = path.join(app.getPath('userData'), 'recordings');
      const metaPath = path.join(recordingsPath, `${recordId}.meta.json`);
      ...
      ) as WorkflowRecordingFileMetadata;
...
export { RecordingHandlers };
export default recordingHandlers;
