...
const WorkspaceModal = ({ visible, editingWorkspace, onCancel, onSuccess }: WorkspaceModalProps) => {
  ...
  const [connectionProgress, setConnectionProgress] = useState<
    { step: string; status: string; details?: string; progress?: number }[]
  >([]);
  ...
  const handleTestConnection = async () => {
    ...
    const isValidUrl =
      /^(https?:\/\/|git@|ssh:\/\/|file:\/\/|\/|\.\/|\.\.\/|[a-zA-Z]:\\|[a-zA-Z]:\/)/i.test(gitUrl) ||
      gitUrl.includes('.git') ||
      gitUrl.includes('@') ||
      gitUrl.startsWith('/') ||
      gitUrl.startsWith('.');
      ...
      form.setFields([
        {
          name: 'gitUrl',
          errors: [
            'Please enter a valid Git repository URL (e.g., https://github.com/user/repo.git, git@github.com:user/repo.git, or /path/to/repo)',
          ],
        },
      ]);
      return;
              ...
              <Space orientation="vertical" style={{ width: '100%' }}>
                ...
                <Radio value={FILE_FORMATS.SEPARATE} disabled={watchedEnvironmentOption === ENVIRONMENT_OPTIONS.NONE}>
                  <Space>
                    ...
                    <div>
                      <Text
                        strong={watchedEnvironmentOption !== ENVIRONMENT_OPTIONS.NONE}
                        disabled={watchedEnvironmentOption === ENVIRONMENT_OPTIONS.NONE}
                      >
                        Multiple Files
                      </Text>
                      <br />
                  ...
                  <>
                    ...
                    >
                      <div style={{ fontSize: '13px', color: '#0050b3' }}>
                        <strong>Note:</strong> When you create this workspace, your current configuration will be
                        committed to the repository at the specified path. This allows your team to share the same
                        settings.
                      </div>
                    </div>
                    {renderExportOptions()}
      ...
      <Alert
        ...
        description={
          <div>
            ...
            {canRetry && (
              <Button size="small" type="primary" onClick={retryCreation} style={{ marginTop: 8 }}>
                Retry Creation
              </Button>
            )}
          </div>
      ...
      return (
        <div style={{ width: '100%' }}>
          ...
          >
            ...
            <span style={{ color: '#1890ff', fontWeight: 500 }}>{progressMessage}</span>
          </div>

          <Space style={{ width: '100%', justifyContent: 'flex-end' }}>
            {canAbort && <Button onClick={handleCancel}>Cancel</Button>}
          </Space>
        </div>
      );
    }

    const showTestButton = (editingWorkspace && watchedGitUrl) || (!editingWorkspace && watchedType === 'team');
      ...
      <Space style={{ width: '100%', justifyContent: 'flex-end' }}>
        ...
        {showTestButton && (
          <Tooltip title="Test repository connection and validate configuration">
            ...
            >
              {isTestingConnection ? 'Testing...' : connectionTested ? 'Connection Verified' : 'Test Connection'}
            </Button>
          </Tooltip>
        )}
        ...
        <Tooltip
          title={
            requireConnectionTest ? 'Please test the Git connection first before creating the workspace' : undefined
          }
        >
          <Button
            type="primary"
