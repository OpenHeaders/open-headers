...
interface ExportModalProps {
  visible: boolean;
  onCancel: () => void;
  onExport: (config: ExportOptions) => void;
}
const ExportModal = ({ visible, onCancel, onExport }: ExportModalProps) => {
  ...
  const handleExport = () => {
    ...
    onExport({
      ...
      selectedEnvironments:
        environmentOption === 'schema' || environmentOption === 'full' ? selectedEnvNames : undefined,
  ...
  const isExportDisabled =
    (!selectedItems.rules && !selectedItems.sources && !selectedItems.proxyRules) ||
      ...
      >
        ...
        >
          <Space orientation="vertical" style={{ width: '100%' }} size="large">
            ...
            {/* What to Export */}
            <ExportItemsSelector selectedItems={selectedItems} onItemChange={handleItemChange} />
        ...
        >
          <Space>
            ...
            <Button type="primary" icon={<ExportOutlined />} disabled={isExportDisabled} onClick={handleExport}>
              Export
            </Button>
          </Space>
        </div>
