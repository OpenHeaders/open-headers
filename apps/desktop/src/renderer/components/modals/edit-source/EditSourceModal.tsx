...
const EditSourceModal = ({ source, open, onCancel, onSave, refreshingSourceId }: EditSourceModalProps) => {
  ...
  useEffect(() => {
    if (open && source) {
      ...
      const requestOptions = {
        ...
        headers: Array.isArray(source.requestOptions?.headers) ? source.requestOptions.headers : [],
      };
  ...
  const handleTotpChange = (enabled: boolean, secret: string) => {
    if (isInitializedRef.current && (enabled !== totpEnabled || (enabled && secret !== totpSecret))) {
  ...
  useEffect(() => {
    if (
      ...
      setTimeout(() => {
        ...
        if (urlValue && typeof urlValue === 'string' && (urlValue.includes('{{') || urlValue.includes('[['))) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (refreshNow && totpEnabled && totpSecret && source.sourceId && !canUseTotpSecret(source.sourceId)) {
      ...
      const submissionHandler = new FormSubmissionHandler(form, source, envContext, httpOptionsRef);
  ...
  const handleTestResponse = (response: string | null) => {
    if (response) {
      try {
        ...
        if (parsedResponse.body && parsedResponse.filteredWith && form.getFieldValue(['jsonFilter', 'enabled'])) {
