...
const CliServerControls = ({
  ...
  const highlightCmd = (cmd: string) => {
    const result = [];
    const c = (color: string, text: string, extra?: React.CSSProperties) => (
      <span key={result.length} style={{ color, ...extra }}>
        {text}
      </span>
    );
    ...
    while ((match = regex.exec(cmd)) !== null) {
      ...
      if (match[1])
        result.push(c('#7dd3fc', match[1])); // curl command
      else if (match[2])
        result.push(c('#64748b', match[2]), '\n'); // \ continuation
      else if (match[3])
        result.push(c('#fbbf24', match[3])); // -H -X -d flags
      else if (match[4])
        result.push(c('#c084fc', match[4])); // HTTP methods
      else if (match[5])
        result.push(c('#93c5fd', match[5], { textDecoration: 'underline' })); // URLs
      else if (match[6])
        result.push(c('#93c5fd', match[6]), ':'); // JSON keys
      else if (match[7])
        result.push(c('#a5d6a7', match[7])); // string values
      else if (match[8])
        result.push(c('#fbbf24', match[8])); // booleans/null
      else if (match[9])
        result.push(c('#94a3b8', match[9])); // braces
  ...
  const highlightPs = (cmd: string) => {
    ...
    while ((match = regex.exec(cmd)) !== null) {
      if (match.index > last) result.push(cmd.slice(last, match.index));
      if (match[1])
        result.push(c('#7dd3fc', match[1])); // cmdlets
      else if (match[2])
        result.push(c('#64748b', match[2]), '\n'); // ` continuation
      else if (match[3])
        result.push(c('#fbbf24', match[3])); // -Parameters
      else if (match[4])
        result.push(c('#67e8f9', match[4])); // $variables
      else if (match[5])
        result.push(c('#93c5fd', match[5], { textDecoration: 'underline' })); // URLs
      else if (match[6])
        result.push(c('#93c5fd', match[6]), '='); // hash keys
      else if (match[7])
        result.push(c('#a5d6a7', match[7])); // string values
      else if (match[8])
        result.push(c('#c084fc', match[8])); // HTTP methods
              ...
              <Space size={4}>
                ...
                <Tooltip title="Reveal in file manager">
                  <Button
                    ...
                    onClick={() => window.electronAPI.showItemInFolder(status.discoveryPath ?? '')}
                  />
                </Tooltip>
              </Space>
            )}
            ...
            description={
              <Space orientation="vertical" style={{ width: '100%' }}>
                ...
                <Space size={12} align="center">
                  <Text strong>
                    <InfoCircleOutlined /> Available Endpoints
                  </Text>
                  {isWindows && (
                    <Segmented
                      size="small"
                  ...
                  items={[
                    {
                      ...
                      label: (
                        <Text>
                          <Text code>GET /cli/health</Text> — Check if the app is running and ready.
                        </Text>
                      ),
                      children: <CodeBlock cmd={cmds.health} />,
                    },
                    {
                      key: 'workspace-join',
                      label: (
                        <Text>
                          <Text code>POST /cli/workspace/join</Text> — Join a team workspace by git repo URL.
                        </Text>
                      ),
                      children: (() => {
                        return (
                            ...
                            <table style={paramTableStyle}>
                              ...
                              <tbody>
                                <tr>
                                  ...
                                  <td style={tdReqStyle}>Yes</td>
                                  <td style={tdStyle}>Git repository URL</td>
                                </tr>
                                <tr>
                                  <td style={tdStyle}>
                                    <Text code style={paramCodeStyle}>
                                ...
                                <tr>
                                  ...
                                  <td style={tdOptStyle}>No</td>
                                  <td style={tdStyle}>
                                    Credentials for authType. Token:{' '}
                                    <Text code style={paramCodeStyle}>{`{"token", "tokenType"}`}</Text>. SSH:{' '}
                                    <Text code style={paramCodeStyle}>{`{"sshKey", "sshPassphrase"}`}</Text>. Basic:{' '}
                                    <Text code style={paramCodeStyle}>{`{"username", "password"}`}</Text>
                                  </td>
                                </tr>
                                <tr>
                                  <td style={tdStyle}>
                                  ...
                                  <td style={tdOptStyle}>No</td>
                                  <td style={tdStyle}>Name of the person who shared the invite</td>
                                </tr>
                              </tbody>
                            </table>
                          </>
                    ...
                    },
                    {
                      ...
                      label: (
                        <Text>
                          <Text code>POST /cli/environments/import</Text> — Import environment variables into the active
                          workspace.
                        </Text>
                      ),
                      children: (() => {
                            ...
                            <table style={paramTableStyle}>
                              ...
                              <tbody>
                                <tr>
                                  ...
                                  <td style={tdReqStyle}>Yes</td>
                                  <td style={tdStyle}>Object mapping environment names to their variables</td>
                                </tr>
                                <tr>
                                  <td style={tdStyle}>
                                    <Text code style={paramCodeStyle}>
                                  ...
                                  <td style={tdReqStyle}>Yes</td>
                                  <td style={tdStyle}>Object mapping variable names to their config</td>
                                </tr>
                                <tr>
                                  <td style={tdStyle}>
                                    <Text code style={paramCodeStyle}>
                                  ...
                                  <td style={tdReqStyle}>Yes</td>
                                  <td style={tdStyle}>The variable value string</td>
                                </tr>
                                <tr>
                                  <td style={tdStyle}>
                                    <Text code style={paramCodeStyle}>
