...
function validateConfigStructure(data: ConfigData) {
  ...
  if (data.proxyRules) {
    ...
    data.proxyRules.forEach((rule, index: number) => {
      ...
      if (!isDynamicRule && !isStaticRule) {
        throw new Error(
          `Invalid proxy rule at index ${index}: must have either domains (for static rules) or headerRuleId (for dynamic rules)`,
        );
      }
  ...
  if (data.environmentSchema) {
    ...
    if (data.environmentSchema.variableDefinitions && typeof data.environmentSchema.variableDefinitions !== 'object') {
...
async function validateGitWorkspaceConfig(content: string, _filePath: string) {
  try {
    ...
    const hasData =
      validationResult.sourceCount > 0 ||
...
export { analyzeConfigFile, readAndValidateMultiFileConfig, validateGitWorkspaceConfig };
