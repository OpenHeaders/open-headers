 ...
 * `electron.vite.config.ts` (gated by `OH_DESKTOP_CHANNEL=release`);
 * local production builds — including the Playwright e2e target — use
 * the real runtime and keep the ids. Catches every origin — renderer
 * JSX, prebuilt `@openheaders/ui` dist, spread props — because all of
 * them create elements through this runtime. The pass-through imports
 * resolve to React's real module via the `react/jsx-runtime-actual`
 * alias.
 */
