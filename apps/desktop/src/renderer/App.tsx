...
const AppComponent: React.FC = () => {
  // Core hooks
  const { sources, addSource, removeSource, refreshSource, updateSource, exportSources } = useSources();
  ...
  const { environments, generateEnvironmentSchema, createEnvironment, setVariable } = useEnvironments();
  ...
  const removeSourceWithRefresh = useCallback(
  ...
  const handleTabScrollPositionChange = useCallback((tab: string, scrollTop: number) => {
  ...
  const handleEnvironmentConfigImport = useCallback(
    async (envData: Partial<EnvironmentConfigData>) => {
      if (envData) {
        try {
          ...
          showMessage(
            'error',
            `Failed to process environment configuration: ${error instanceof Error ? error.message : String(error)}`,
          );
        }
      }
    },
    ...
    <>
      ...
      >
        <AppLayout
          ...
          onHandleExport={(config) => {
            void handleExport(config as Parameters<typeof handleExport>[0]);
          }}
