/**
 * Sandboxed script runner — the desktop twin of the extension's
 * `offscreen/sandbox.ts` (ARCHITECTURE §19), running inside a hidden
 ...
 *
 * We never touch storage / network / host APIs directly. Every
 * side-effecting `oh.*` call posts a `script.host-request` and awaits
 * the reply; the main-process broker is the only thing that speaks to
 * the engine.
 *
...
async function runScript(req: ScriptExecutionRequest): Promise<ScriptExecutionResult> {
  ...
  try {
    ...
    // back to the sandboxed window object (no Node, no host APIs, only
    // postMessage on a hidden window — defense in depth).
