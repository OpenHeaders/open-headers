...
describe('bootTranslator', () => {
  ...
  it('renders the boot-beat shell strings byte-exact in English', () => {
    ...
    expect(t('web.insecure.optionTls')).toBe('From here over HTTPS — put a TLS-terminating reverse proxy in front.');
  ...
  it('renders gate copy byte-exact, recomposed around the raw islands', () => {
    ...
    expect(t('web.gate.titleSetup')).toBe('Set up this server');
    expect(t('web.gate.introSso', { provider: 'ACME SSO' })).toBe(
      'Sign in with ACME SSO to reach this OpenHeaders Server.',
    );
    ...
    expect(`${t('web.gate.clientsIntro')} ws://openheaders.io:8137`).toBe(
      'This tab is not the only client. The extension and the desktop app reach this server directly at ws://openheaders.io:8137',
    );
    expect(t('web.gate.clientsExtension')).toBe('Get the extension');
    expect(t('web.gate.clientsDesktop')).toBe('Get the desktop app');
    expect(t('web.gate.errorServerOffline')).toBe('The server did not answer. Check that it is running and try again.');
