 ...
 *   6. The script sandbox is not a served page: it is a self-contained
 *      document the tab carries and mounts inline (the Execution Place
 *      epic's F23 — Chromium routes no opaque-origin frame's navigation
 *      through the service worker, so a served page could never come
 *      back from the cache), so nothing here is precached or served for
 *      it; the desktop web-rig spec proves the mounted realm runs.
 *
