...
| Symptom | Cause | Fix |
| --- | --- | --- |
| The setup card answers `The server refused the setup. It may already be set up, or the setup code may be wrong or left over from an earlier start — the server prints a new one every time it restarts.` | One refusal covers every state-dependent reason on purpose, so it cannot say which | Read the current code from `ohd status` (a restart replaces it), confirm the server is still unclaimed there, and retry. Case and dashes do not matter |
| The setup card asks for a code even though the browser IS on the server | The daemon sees the request arriving from a reverse proxy, not from loopback | Set `--trusted-proxy` so the peer is taken from `X-Forwarded-For` — see [LAN vs TLS](/server/lan-vs-tls) — or just enter the code from `ohd status` |
| A browser reaching a container gets asked for a code | Docker's bridge address is not loopback to the daemon, whatever the host URL says | Expected — use the code from `docker logs <container>` |
| The card says `Sign in to this server` but nobody ever created an account | The server is already claimed — the setup screen never comes back | `ohd user list` on the server (daemon stopped) shows who holds it; set a password for yourself with `ohd user set-password` |
| The card says `This server has no way for a browser to sign in: single sign-on is not configured, and no account on it has a password.` | Claimed, no identity provider, and no account left holding a password | `ohd user set-password <id-or-email>` (daemon stopped), then sign in |
| Paired devices stopped working immediately after the claim | The claim revokes every **unbound** token — they acted as the server operator, which would bypass the new admin | Expected; pair them again from the admin console. Bind future bootstrap tokens with `ohd show-token --user <id-or-email>` |
| `ohd status` prints no claim block | The server is already claimed (or was claimed mid-run — the block clears without a restart) | Nothing to do; sign in instead |
| Nobody can administer the server any more | The last admin was deactivated — deactivation is deliberately not guarded, only demotion is | `ohd stop`, `ohd user set-admin <id-or-email>`, `ohd start` |
...
| A minted token was never saved | `show-token` and the console both show the secret once | Mint another from the admin console under **Paired devices**, or run `ohd show-token` again (daemon stopped) |
...
| `'…' is the only server admin — grant the role to someone else first, or the server would be left with no one who can administer it.` | Revoking the last server-admin role | Grant it to another user first; the check counts holders, not who is asking |
