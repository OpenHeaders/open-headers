...
| The served web app in a browser | Only on the server's own machine, at `http://127.0.0.1:8137/` | From any browser, at `https://<your-host>/` |
...
(`ufw`/`firewalld`) admits port 8137 — the daemon logs every connection
it refuses, so a client that fails while the log stays silent never
