...
| Extension, desktop, CLI clients | Work from any machine on the network | Work from anywhere the domain resolves |
| The served web app in a browser | Only on the server's own machine, at `http://127.0.0.1:8137/`, or through an SSH tunnel to it | From any browser, at `https://<your-host>/` |
| [Claiming the server](/server/first-run) | On the server itself, with no setup code | From any browser, with the setup code from `ohd status` |
...
The web-app row is the browser's rule, not the daemon's: the Workbench
needs `crypto.subtle` to mint its identity, and browsers withhold it on
plain-HTTP origins that are not loopback, so a LAN URL like
`http://<server>:8137/` loads and then refuses to start. It binds the
served web app alone — the extension and desktop app speak
`ws://<server>:8137` in either posture, which is why a headless server
with no browser on it is a perfectly ordinary deployment.
...
`ohd status` lists the LAN join URLs the running daemon actually
answers at. If clients still cannot connect, check that the host
firewall (`ufw`/`firewalld`) admits port 8137 — the daemon logs every
connection it refuses, so a client that fails while the log stays
silent never reached the process at all.
...
- `--trusted-proxy` makes auth logs, rate limits, and the
  [claim](/server/first-run)'s loopback test use the client address the
  proxy appends to `X-Forwarded-For` instead of the proxy's own —
  without it a proxy on the same machine would make every remote
  browser look local and the setup code would gate nothing. **Never set
  it without a proxy in front** — clients could then spoof the header.
