...
```sh
docker run -d -p 8137:8137 -v oh-data:/data ghcr.io/openheaders/ohd:2026.8.3
```
...
services:
  ohd:
    image: ghcr.io/openheaders/ohd:2026.8.3
...
- **First token**: `ohd show-token` needs the daemon stopped and the
  ...
  docker run --rm -v oh-data:/data ghcr.io/openheaders/ohd:2026.8.3 show-token --data-dir /data
...
- **Settings** work the same way (stopped daemon, same volume):
  ...
  ```sh
  docker run --rm -v oh-data:/data ghcr.io/openheaders/ohd:2026.8.3 config set mcp.enabled true --data-dir /data
  ```
...
- **Backup**: stop the container and snapshot with the same tooling —
  ...
  docker run --rm -v oh-data:/data -v "$PWD/backups:/backups" \
    ghcr.io/openheaders/ohd:2026.8.3 backup /backups/oh-2026-08-22 --data-dir /data
  ```
