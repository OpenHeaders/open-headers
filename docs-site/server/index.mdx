...
<CardGroup cols={2}>
  ...
  <Card title="First run: claim it" href="/server/first-run">
    A fresh server has no administrator. The first browser to reach it
    creates one — and what that claim does to existing tokens.
  </Card>
  ...
  <Card title="Tokens & pairing" href="/server/tokens-and-pairing">
    Where client credentials come from once the server is claimed, and
    the admission rules every route enforces.
  </Card>
  <Card title="Users, seats & SSO" href="/server/users-sso">
    The user directory, the server-admin role, workspace grants, seat
    limits, passwords, and OpenID Connect login.
  </Card>
