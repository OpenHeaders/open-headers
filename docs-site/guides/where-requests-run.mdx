...
| gRPC | Forwarded to the connected desktop app, else to the workspace's server — the browser has no HTTP/2 stack that exposes trailers. With both connected, the desktop app runs it and the server is offered beside it. Without either: **Needs the desktop app**. | Here. | Sent by the server and resolved there. |
...
workspace's server connected — reads **Needs the desktop app**. The
chip's action follows what is installed:
