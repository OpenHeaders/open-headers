...
const FIELD_ROWS = {
  ...
  sessionTtlDays: ['number', '`30`', '`OH_DAEMON_SESSION_TTL_DAYS`', '—', 'How many days a person\'s session lasts — a browser login, the server claim, an SSO completion, and a device signed in through the server\'s page alike. One policy for every session; whole days, at least 1.'],
  ...
  publicWorkspaces: ['boolean', '`false`', '`OH_PUBLIC_WORKSPACES`', '—', 'Public snapshot plane master switch. On, owners of `public`-visibility workspaces can publish a read-only snapshot served anonymously at `/public/<workspaceId>`. Off gates both publishing and serving — standing publications go dark, nothing is deleted.'],
  ...
  useSystemCa: ['boolean', 'not set (the device\'s stored answer stands)', '`OH_DAEMON_USE_SYSTEM_CA`', '`--use-system-ca` / `--no-use-system-ca`', 'Seeds this machine\'s opt-in to the operating system\'s certificate store at boot (additive — the OS roots ride beside the bundled ones). `true` / `false` writes the device posture; absent leaves the stored answer alone.'],
...
const ENV_ROWS = {
  ...
  OH_DAEMON_SESSION_TTL_DAYS: 'Session lifetime override, in whole days.',
  ...
  OH_PUBLIC_WORKSPACES: '`1`/`0` — public snapshot plane master switch.',
  OH_DAEMON_USE_SYSTEM_CA: '`1`/`0` — system certificate store opt-in seed.',
