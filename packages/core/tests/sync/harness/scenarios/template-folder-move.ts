/**
 * Template-folder reorder + reparent under contention.
 *
 * Mirrors `request-folder-move.ts` against the template-folder entity
 * types (`template-collection` parent, `template-folder` child slot).
 * The convergence engine doesn't care about routing keys per se — this
 * bucket is explicit coverage that the new types pass through the
 * per-(setPath, itemId) LWW path with the same determinism.
 *
 * Three folders seeded as children of one parent template collection,
 * then two surfaces concurrently move different folders while a third
 * surface reparents one of them under a sibling template collection.
 */
...
const PARENT_TYPE = 'template-collection';
const SLOT_PATH = 'folders';

export function genTemplateFolderMove(rng: Rng): Scenario {
  const parentA = rng.uid('tcol');
  const parentB = rng.uid('tcol');
  const ws = 'ws-1';
  ...
  const fAlpha = rng.uid('tf');
  const fBeta = rng.uid('tf');
  const fGamma = rng.uid('tf');
  ...
  return {
    name: `template-folder-move(${fAlpha},${fBeta},${fGamma}@${parentA}→${parentB})`,
    envelopes: [
      seedAlpha,
      seedBeta,
      seedGamma,
