...
const hostCondition: RuleCondition = { type: 'host', operator: 'contains', values: ['openheaders.io'] };
const base = { uid: 'x1', path: 'rules/col-abc1/rule-x1', name: 'Test', enabled: true, conditions: [hostCondition] };
...
describe('isRuleComplete', () => {
  ...
  it('returns false when all condition values are whitespace', () => {
    const emptyCondition: RuleCondition = { type: 'host', operator: 'contains', values: ['  ', ''] };
    expect(isRuleComplete({ ...base, conditions: [emptyCondition], type: 'block', action: { statusCode: 403 } })).toBe(false);
  });
  ...
  it('returns true with multiple conditions', () => {
    const methodCondition: RuleCondition = { type: 'method', operator: 'equals', values: ['GET', 'POST'] };
    expect(isRuleComplete({ ...base, conditions: [hostCondition, methodCondition], type: 'block', action: { statusCode: 403 } })).toBe(true);
  });

  it('returns false when condition values array is empty', () => {
    const noValues: RuleCondition = { type: 'host', operator: 'contains', values: [] };
  ...
  it('header: complete with name + value', () => {
    expect(isRuleComplete({ ...base, type: 'header', action: { operation: 'override', headerName: 'X-Debug', isResponse: false, value: 'true' } })).toBe(true);
  });

  it('header: incomplete without headerName', () => {
    expect(isRuleComplete({ ...base, type: 'header', action: { operation: 'override', headerName: '', isResponse: false, value: 'true' } })).toBe(false);
  });

  it('header: incomplete without value for add/override', () => {
    expect(isRuleComplete({ ...base, type: 'header', action: { operation: 'add', headerName: 'X-Debug', isResponse: false, value: '' } })).toBe(false);
  });

  it('header: complete without value for remove operation', () => {
    expect(isRuleComplete({ ...base, type: 'header', action: { operation: 'remove', headerName: 'X-Debug', isResponse: false, value: '' } })).toBe(true);
  });
  ...
  it('works on Omit<Rule, uid | path> for pre-save checks', () => {
    const partial = { name: 'Draft', type: 'header' as const, enabled: true, conditions: [hostCondition], action: { operation: 'override' as const, headerName: 'X-Test', isResponse: false, value: 'val' } };
    expect(isRuleComplete(partial)).toBe(true);
  });

  it('empty draft rule is incomplete', () => {
    const partial = { name: 'New Header Rule', type: 'header' as const, enabled: true, conditions: [], action: { operation: 'override' as const, headerName: '', isResponse: false, value: '' } };
  ...
  it('exclude condition with values is valid', () => {
    const excludeCondition: RuleCondition = { type: 'host', operator: 'contains', values: ['staging.openheaders.io'], exclude: true };
    ...
    expect(isRuleComplete({ ...base, conditions: [hostCondition, excludeCondition], type: 'block', action: { statusCode: 403 } })).toBe(true);
  });

  // ── Header condition types ──────────────────────────────────────

  it('request-header condition with headerName is valid', () => {
    const headerCondition: RuleCondition = { type: 'request-header', operator: 'contains', values: ['Bearer*'], headerName: 'Authorization' };
    expect(isRuleComplete({ ...base, conditions: [headerCondition], type: 'block', action: { statusCode: 403 } })).toBe(true);
  });
});
