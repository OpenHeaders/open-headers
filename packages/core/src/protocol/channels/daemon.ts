...
export interface DaemonRpc {
   ...
   * profiles or installs) unioned with the recorded changes; `changes`
   * is the durable what-we-changed record teardown will undo. Where
   * Firefox reads the OS store (macOS, Firefox 120+), each installed
   * app bundle carries a derived verdict (`covered`) instead of a
