/**
 * Internal helpers — mint envelopes + batches from extensionWorkspace
 * mutator arguments. Mirrors `files/envelope.ts`.
 */
...
/** ExtensionWorkspace mutator catalog version — bump on any wire-incompatible change (§13.4). */
export const EXTENSION_WORKSPACE_MUTATOR_VERSION = 1;
...
export function mintEnvelope(ctx: MutatorContext, body: MutationBody): MutationEnvelope {
  return {
    ...
    mutatorVersion: EXTENSION_WORKSPACE_MUTATOR_VERSION,
    body,
  };
}
