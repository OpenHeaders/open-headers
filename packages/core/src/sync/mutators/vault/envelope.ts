/**
 * Internal helpers — mint envelopes + batches from vault-mutator
 * arguments. Mirrors `workspace-variables/envelope.ts`.
 */
...
/** Vault mutator catalog version — bumped on any wire-incompatible change (§13.4). */
export const VAULT_MUTATOR_VERSION = 1;
...
export function mintEnvelope(ctx: MutatorContext, body: MutationBody): MutationEnvelope {
  return {
    ...
    mutatorVersion: VAULT_MUTATOR_VERSION,
    body,
  };
}
