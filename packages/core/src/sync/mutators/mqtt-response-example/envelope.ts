/**
 * Internal helpers — mint envelopes + batches from MQTT
 * response-example mutator arguments. Mirrors the other catalogs.
 */
...
/** MQTT response-example mutator catalog version — bump on any wire-incompatible change (§13.4). */
export const MQTT_RESPONSE_EXAMPLE_MUTATOR_VERSION = 2;
...
export function mintEnvelope(ctx: MutatorContext, body: MutationBody): MutationEnvelope {
  return {
    ...
    mutatorVersion: MQTT_RESPONSE_EXAMPLE_MUTATOR_VERSION,
    body,
  };
}
