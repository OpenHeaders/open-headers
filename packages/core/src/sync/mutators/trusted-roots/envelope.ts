/**
 * Internal helpers — mint envelopes + batches from trusted-roots
 * mutator arguments. Mirrors `pause-markers/envelope.ts`.
 */
...
/** Trusted-roots mutator catalog version — bumped on any wire-incompatible change (§13.4). */
export const TRUSTED_ROOTS_MUTATOR_VERSION = 1;
...
export function mintEnvelope(ctx: MutatorContext, body: MutationBody): MutationEnvelope {
  return {
    ...
    mutatorVersion: TRUSTED_ROOTS_MUTATOR_VERSION,
    body,
  };
}
