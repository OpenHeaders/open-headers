/**
 * Internal helpers — mint envelopes + batches from gRPC response-example
...
/** gRPC response-example mutator catalog version — bump on any wire-incompatible change (§13.4). */
export const GRPC_RESPONSE_EXAMPLE_MUTATOR_VERSION = 1;
...
export function mintEnvelope(ctx: MutatorContext, body: MutationBody): MutationEnvelope {
  return {
    ...
    mutatorVersion: GRPC_RESPONSE_EXAMPLE_MUTATOR_VERSION,
    body,
  };
}
