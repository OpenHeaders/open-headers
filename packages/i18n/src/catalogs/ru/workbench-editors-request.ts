...
export const workbenchEditorsRequest = {
  ...
  'workbench.editors.request.settings.executionPlaceInfo':
    'Где открывается соединение этого запроса: на этом устройстве, в настольном приложении или на сервере рабочего пространства. Запрос по-прежнему разрешается здесь; перемещается только соединение. «Автоматически» выполняет его здесь, когда это устройство может, иначе — в единственном месте, которое может.',
  ...
  'workbench.editors.request.settings.managed.scriptsSafeHereDesc':
    'Скрипты перед запросом и после ответа выполняются здесь, в изолированной безопасной среде этой вкладки: только API скриптов oh.* — без файловой системы, без доступа к процессам, без загрузчика модулей. У вкладки браузера нет режима разработчика, и каждый запуск записывает на ответе режим, в котором выполнялся.',
