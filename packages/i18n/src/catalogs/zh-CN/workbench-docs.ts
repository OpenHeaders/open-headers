...
export const workbenchDocs = {
  ...
  'workbench.docs.body.paradigm.localFirst1Suffix':
    '这件事上有真正的选择。四种托管方式，全部仅限本地、全部在你掌控之下：浏览器内 Service Worker（今天即可，零设置）、桌面端应用的内嵌后端、在一台机器上服务所有 Open Headers 界面的独立本地守护进程，或你在自己 VM 上自托管的后端。每种方式都保有同样的保证；取舍在于触达范围，而不是所有权。',
  ...
  'workbench.docs.body.paradigm.autoSync1Suffix':
    '标志，工作区编辑器改写同一条规则里的标头值——两者以任意顺序都能落地，没有过期草稿横幅，没有覆盖。同一套方案从一个扩展的四个界面，扩展到一个本地守护进程支撑扩展 + 桌面 + CLI，再到经由 Git 远程的多用户团队工作区——中间始终不需要供应商的服务器。',
  ...
  'workbench.docs.body.roadmap.daemonHeading': '用于跨设备同步的本地 / LAN 守护进程',
  'workbench.docs.body.roadmap.daemon1':
    '一个可以跑在你的机器、你的 LAN 或一台隧道主机上的同步守护进程。扩展、桌面端应用和 CLI 都成为同一个守护进程的客户端——同样的工作区、同样的规则、同样的 vault，横跨你使用的每台设备。守护进程留在本地网络上；没有叠加在上面的可选云通道。',
  'workbench.docs.body.roadmap.cliHeading': 'CLI',
  'workbench.docs.body.roadmap.cli1':
    '无界面脚本与 CI 集成。列出规则、切换环境、从 shell 运行单个已保存的请求、把一个工作区与另一个做对比。CLI 与扩展和桌面端应用对话同一个守护进程，因此自动化与你在 UI 中看到的保持同步。',
