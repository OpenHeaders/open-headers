...
export const workbenchEditorsWebsocket = {
  ...
  'workbench.editors.websocket.connect.browserHost': 'WebSocket 会话在桌面端应用或守护进程上运行。',
  ...
  'workbench.editors.websocket.auth.helpRaw':
    '在握手时作为 Authorization: Bearer 标头发送——适用于桌面端应用或守护进程；浏览器无法在 WebSocket 上设置它。' +
    '显式的 Authorization 标头行优先。',
  'workbench.editors.websocket.auth.helpSocketio':
    '在每种主机上作为 CONNECT 包的 auth 负载（{"token": …}）发送，并在桌面端应用或守护进程上作为 ' +
  ...
  'workbench.editors.websocket.headers.nodeOnly':
    '自定义握手标头在会话运行于桌面端应用或守护进程时适用——浏览器无法在 WebSocket 上设置它们。',
  ...
  'workbench.editors.websocket.settings.sslVerifyHelp':
    '对 wss: 会话按系统根证书验证服务器证书。对自签名的开发服务器可关闭。适用于桌面端应用或守护进程。',
