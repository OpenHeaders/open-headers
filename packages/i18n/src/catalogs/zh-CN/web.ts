...
export const web = {
  ...
  'web.gate.titlePair': '与此服务器配对',
  'web.gate.introSso': '使用 {provider} 登录，或在下方粘贴配对 token。',
  'web.gate.introPassword': '使用服务器管理员为你设置的邮箱和密码登录，或在下方粘贴配对 token。',
  'web.gate.introTokenPrefix': '此 OpenHeaders 服务器需要配对 token。在运行它的机器上使用',
  ...
  'web.gate.connect': '连接',
  'web.gate.clientsIntro': '此标签页并非唯一的客户端。使用同一个配对 token，扩展程序和桌面应用可直接连接到',
  'web.gate.clientsExtension': '获取扩展程序',
  'web.gate.clientsDesktop': '获取桌面应用',
  'web.gate.errorTokenRejected': '服务器拒绝了此 token。请检查后重试。',
  'web.gate.errorTokenOffline': '服务器没有响应。请检查它是否在运行，然后重试。',
