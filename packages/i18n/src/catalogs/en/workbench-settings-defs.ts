/**
 * Workbench settings — the setting-definition corpus: every
 * registered setting's label/description and enum option copy, keys
 * derived from the setting key
 * (`workbench.settings.def.<category>.<setting>.*`). Grows with the
 * settings registry (`packages/ui/src/workbench/settings/schema/`).
 *
