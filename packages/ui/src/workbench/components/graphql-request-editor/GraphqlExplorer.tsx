/**
 * GraphqlExplorer — the Query tab's docs explorer over the resolved
 * schema: the root operation types and their fields, navigation into
 * any type (fields with arguments and return types, enum values, input
 * fields, interfaces and possible types, a scalar's specification
 * URL), a field page with its arguments and deprecation, a search
 * across type and field names, and a one-way "Insert at cursor" that
 * adds a field (with its required arguments as variables and an empty
 * selection set when it returns a composite type) to the document —
 * the reference's docs pane; the two-way builder is Phase F's.
 *
...
import { Button, Input, Tag, Tooltip, Typography, theme } from 'antd';
...
const GraphqlExplorer: React.FC<GraphqlExplorerProps> = ({ schema, onInsert }) => {
  ...
  if (query !== '') {
    ...
    body = ROOT_OPERATIONS.map((operation) => {
      const name = rootTypeName(schema, operation);
      const type = name === null ? undefined : schema.types.get(name);
      if (name === null || type === undefined || (type.kind !== 'OBJECT' && type.kind !== 'INTERFACE')) return null;
      return (
        <div key={operation} data-testid={`graphql-explorer-root-${operation}`}>
          {sectionTitle(operation)}
          <div style={{ marginBottom: 2 }}>{typeLink(name)}</div>
          {type.fields.map((field) => fieldRow(name, field))}
        </div>
      );
    });
