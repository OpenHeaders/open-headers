...
export const FileTypeIcon: React.FC<{ path: string }> = ({ path }) => {
  ...
  if (name === '.gitignore' || name === '.gitattributes') {
    return <StopOutlined style={{ ...GLYPH_STYLE, fontSize: 12, color: token.colorTextTertiary }} />;
  }
  if (ext === 'json') return <span style={{ ...GLYPH_STYLE, color: '#9876AA' }}>{'{}'}</span>;
  if (ext === 'xml' || ext === 'html' || ext === 'svg') {
    return <span style={{ ...GLYPH_STYLE, color: '#E8A33D' }}>{'</>'}</span>;
  }
  if (ext === 'js' || ext === 'mjs' || ext === 'cjs') return <span style={{ ...GLYPH_STYLE, color: '#E9CA4B' }}>JS</span>;
  if (ext === 'ts' || ext === 'tsx') return <span style={{ ...GLYPH_STYLE, color: '#548AF7' }}>TS</span>;
  if (ext === 'png' || ext === 'jpg' || ext === 'jpeg' || ext === 'gif' || ext === 'webp' || ext === 'ico') {
    return <FileImageOutlined style={{ ...GLYPH_STYLE, fontSize: 12, color: token.colorTextTertiary }} />;
  }
  if (ext === 'yaml' || ext === 'yml' || ext === 'md' || ext === 'txt' || ext === 'log') {
    return <FileTextOutlined style={{ ...GLYPH_STYLE, fontSize: 12, color: token.colorTextTertiary }} />;
  }
  return <FileOutlined style={{ ...GLYPH_STYLE, fontSize: 12, color: token.colorTextTertiary }} />;
};
