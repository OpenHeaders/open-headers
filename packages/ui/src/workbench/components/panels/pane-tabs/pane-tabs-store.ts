/**
 * pane-tabs-store — the shared split-layout owner for tab-strip dock
 * panels (terminal, git log): a recursive leaf tree (the editor-groups
 * model bound to bare tab-id refs), a per-leaf active selection, and
 * the focused leaf. Extracted verbatim from the terminal's pane store
 * so every split-tab panel rides ONE machinery instead of forks.
 *
 * The panel's REGISTRY owns tab identities (what a tab is); this store
 * owns WHERE each tab lives. The registry contract is the minimal
 * five-verb slice the reconcile loop needs ({@link PaneTabsRegistry}).
 *
 * Module-state posture (created by the caller, held by the caller's
 * singleton): the dock unmounts hidden tool windows, and the split
 * layout must survive a hide/show round-trip. Splits do NOT persist
 * across app restarts — the editor tab session has the same contract.
 *
 * Ownership of activation is split by direction:
 *   - registry → panes: tab ADDED — it lands in the focused leaf and
 *     becomes its active tab; a pure registry-side activation focuses
...
export type PaneTabsNode = EditorNode<PaneTabRef>;
export type SplitDirection = 'left' | 'right' | 'top' | 'bottom';

/** The registry slice the store reconciles against. */
export interface PaneTabsRegistry {
  /** Flat tab identities in persisted order. */
  list(): ReadonlyArray<{ id: string }>;
  activeId(): string | null;
  activateTab(id: string): void;
  /** Receives the leaf-concatenated visual order after structural changes. */
...
function pushOrder(state: PanesState, registry: PaneTabsRegistry): void {
  registry.setOrder(allTabs(state.root).map((ref) => ref.id));
}

function applyTransform(
  state: PanesState,
  registry: PaneTabsRegistry,
  fn: (prev: { root: PaneTabsNode; focusedLeafId: string; nextId: number }) => {
    root: PaneTabsNode;
    focusedLeafId: string;
    nextId: number;
  },
 ...
 * owning leaf.
 */
function reconcile(state: PanesState, registry: PaneTabsRegistry): void {
  ...
  try {
    ...
    if (removedAny) {
      ...
      // Pure activation change (e.g. a tab-search dropdown) — follow
...
/**
 * Create a pane store bound to `registry`. The caller owns the
 * instance's lifetime (module singleton per panel — or per workspace,
 * the git panel's keying).
 */
export function createPaneTabsStore(registry: PaneTabsRegistry): WorkbenchPaneTabs {
  const state: PanesState = {
    root: makeLeaf<PaneTabRef>(ROOT_LEAF_ID),
  ...
  return {
    ...
    changeSplitterOrientation: (leafId) => transform((prev) => ({ ...prev, root: flipParentSplit(prev.root, leafId) })),
    unsplit: (leafId) =>
...
export function parentOrientationOf(root: PaneTabsNode, leafId: string): EditorOrientation | null {
...
export function oppositeDirectionOf(root: PaneTabsNode, leafId: string): 'left' | 'right' | 'up' | 'down' | null {
