...
export function makeLeaf(id: string, tabs: WorkbenchTab[] = [], activeTabId: string | null = null): EditorLeaf {
...
export function allTabs(node: EditorNode): WorkbenchTab[] {
 ...
 */
export function insertTabIntoLeaf(root: EditorNode, leafId: string, tab: WorkbenchTab, insertAt?: number): EditorNode {
...
export function updateTabInLeaf(
  ...
  return mapLeaf(root, leafId, (leaf) => {
    ...
    let hasChange = false;
    for (const key of Object.keys(updates) as Array<keyof WorkbenchTab>) {
...
export function replaceTabInLeaf(root: EditorNode, leafId: string, oldTabId: string, newTab: WorkbenchTab): EditorNode {
  return mapLeaf(root, leafId, (leaf) => ({
    ...leaf,
...
export function unsplitLeaf(root: EditorNode, leafId: string): EditorNode {
  ...
  const mergedTabs: WorkbenchTab[] = [...self.tabs];
