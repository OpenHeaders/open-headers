/**
 * EnvironmentProxySection — the environment plane's settings surface
 ...
 * edit applies live over `oh.desktop.environmentProxy.set` — the next
...
const MODE_DESC: Record<DesktopEnvironmentProxyMode, MessageKey> = {
  ...
  off: 'workbench.settings.environmentProxy.mode.offDesc',
};

/** Wire vocabulary for the resolved chain — raw tokens, localized
 *  caption only (the settings example-card precedent). */
function chainText(resolution: EnvironmentProxyResolution | null): string {
...
const EnvironmentProxySection: React.FC = () => {
  const { token } = theme.useToken();
  const t = useT();
  const { vault } = useVaultContext();
  const [settings, setSettings] = useState<EnvironmentProxySettings | null>(null);
  ...
  const refreshProbe = useCallback(async (mode: EnvironmentProxySettings['mode']): Promise<void> => {
    ...
    try {
      const resp = await hostBridge.call('oh.desktop.environmentProxy.resolve', { url: PROBE_URL });
  ...
  useEffect(() => {
    void (async () => {
      try {
        const resp = await hostBridge.call('oh.desktop.environmentProxy.get');
  ...
  const persist = useCallback(
    async (next: EnvironmentProxySettings): Promise<void> => {
      setSettings(next);
      try {
        const resp = await hostBridge.call('oh.desktop.environmentProxy.set', { settings: next });
  ...
  const runPreview = async (): Promise<void> => {
    ...
    setPreviewBusy(true);
    try {
      const resp = await hostBridge.call('oh.desktop.environmentProxy.resolve', { url });
  ...
  const setField = (patch: Partial<EnvironmentProxySettings>): void => {
    ...
    <section style={{ marginBottom: 14 }}>
      <div className="settings-card" style={{ padding: '8px 14px 12px' }}>
        <div style={{ fontSize: 12, fontWeight: 600, color: token.colorText, padding: '4px 0 2px' }}>
          {t('workbench.settings.environmentProxy.title')}
        </div>
        <p style={{ margin: '2px 0 8px', fontSize: 12, color: token.colorTextSecondary }}>
          {t('workbench.settings.environmentProxy.intro')}
        </p>
        ...
        >
          {(['system', 'manual', 'pac', 'off'] as const).map((mode) => (
            <Radio key={mode} value={mode} data-testid={`oh-envproxy-mode-${mode}`} style={{ alignItems: 'flex-start' }}>
        ...
        {settings.mode === 'manual' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6, margin: '10px 0 2px' }}>
            <div style={{ display: 'flex', gap: 12 }}>
              <FieldLabel>{t('workbench.settings.environmentProxy.manual.url')}</FieldLabel>
              <Input
                size="small"
                data-testid="oh-envproxy-manual-url"
                defaultValue={settings.manualProxyUrl ?? ''}
                placeholder={t('workbench.settings.environmentProxy.manual.urlPlaceholder')}
                maxLength={512}
                style={{ maxWidth: 320 }}
                onBlur={(e) => {
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              <FieldLabel>{t('workbench.settings.environmentProxy.manual.credentials')}</FieldLabel>
              <Select
                size="small"
                data-testid="oh-envproxy-manual-credential"
                value={settings.manualCredentialRef}
                onChange={(manualCredentialRef) => setField({ manualCredentialRef })}
                options={credentialOptions}
                allowClear
                placeholder={t('workbench.settings.environmentProxy.manual.credentialsPlaceholder')}
                popupMatchSelectWidth={false}
                style={{ width: 320 }}
              />
            </div>
            <div style={{ display: 'flex', gap: 12 }}>
              <FieldLabel>{t('workbench.settings.environmentProxy.manual.bypass')}</FieldLabel>
              <Input
                size="small"
                data-testid="oh-envproxy-manual-bypass"
                defaultValue={settings.manualBypassList ?? ''}
                placeholder={t('workbench.settings.environmentProxy.manual.bypassPlaceholder')}
                maxLength={2048}
                style={{ maxWidth: 320 }}
                onBlur={(e) => {
                  const value = e.target.value.trim();
        ...
        {settings.mode === 'pac' && (
          <div style={{ display: 'flex', gap: 12, margin: '10px 0 2px' }}>
            <FieldLabel>{t('workbench.settings.environmentProxy.pac.source')}</FieldLabel>
            <Input
              size="small"
              data-testid="oh-envproxy-pac-source"
              defaultValue={settings.pacSource ?? ''}
              placeholder={t('workbench.settings.environmentProxy.pac.sourcePlaceholder')}
              maxLength={1024}
              style={{ maxWidth: 420 }}
              onBlur={(e) => {
                const value = e.target.value.trim();
        ...
        {saveError !== null && (
          <p style={{ margin: '6px 0 0', fontSize: 12, color: token.colorError }}>
            {t('workbench.settings.environmentProxy.saveFailed', { message: saveError })}
          </p>
        )}
        ...
        {settings.mode !== 'off' && (
          <div style={{ display: 'flex', gap: 8, alignItems: 'baseline', marginTop: 10, fontSize: 12 }}>
            <span style={{ color: token.colorTextSecondary, flex: 'none' }}>
              {t('workbench.settings.environmentProxy.sourced', { url: PROBE_URL.replace(/^https:\/\//, '') })}
            </span>
            <span
              data-testid="oh-envproxy-sourced"
            ...
            <Button size="small" type="text" style={{ fontSize: 12, color: token.colorTextSecondary }} onClick={() => void refreshProbe(settings.mode)}>
              {t('workbench.settings.environmentProxy.refresh')}
            </Button>
          </div>
        )}
        ...
        {settings.mode !== 'off' && (
          <div style={{ marginTop: 8 }}>
            <div style={{ display: 'flex', gap: 8 }}>
              <Input
                size="small"
                data-testid="oh-envproxy-preview-url"
                value={previewUrl}
                onChange={(e) => setPreviewUrl(e.target.value)}
                placeholder={t('workbench.settings.environmentProxy.previewPlaceholder')}
                style={{ maxWidth: 320 }}
                onPressEnter={() => void runPreview()}
              />
              <Button size="small" loading={previewBusy} onClick={() => void runPreview()} data-testid="oh-envproxy-preview-run">
                {t('workbench.settings.environmentProxy.previewButton')}
              </Button>
            </div>
            {preview !== null && (
              <div
                data-testid="oh-envproxy-preview-result"
...
export default EnvironmentProxySection;
