...
export async function buildSnapshot(workspaceId: string, service: WorkspaceServiceState): Promise<WorkspaceTreeState> {
  ...
  const src = await hostStorage.getMany({
    ...
    mqttRequests: k.mqttRequests,
    graphqlRequests: k.graphqlRequests,
  ...
  const state: WorkspaceTreeState = {
    ...
    mqttRequests: src.mqttRequests ?? [],
    graphqlRequests: src.graphqlRequests ?? [],
    ...
    trustedRoots: src.trustedRoots ?? null,
    specs: src.specs ?? [],
