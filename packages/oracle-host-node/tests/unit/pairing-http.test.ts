...
describe("pairing HTTP — the client initiative's seam (the client sign-in plan §6)", () => {
  let pairing: DaemonPairingService;

  beforeEach(() => {
    setHostStorage(createHostStorageFake());
    pairing = createDaemonPairingService({ generateCode: () => '135790' });
  });

  it("hands a client pair's GET to the device page owner, already peeked", async () => {
    await pairing.startClientPair({ client: 'extension', peer: '127.0.0.1', deviceLabel: 'Work Chrome' });
    const seen: string[] = [];
    const handler = createPairingHttpHandler({
      pairing,
      clientPairView: (pair, _req, res) => {
        seen.push(`${pair.initiative}:${pair.code}:${pair.deviceLabel ?? ''}`);
        res.statusCode = 200;
        res.end('device page');
      },
    });
    const cap = makeRes();
    expect(handler(makeReq({ method: 'GET', url: '/pair/135790' }), cap.res)).toBe(true);
    await cap.done;
    expect(seen).toEqual(['client:135790:Work Chrome']);
    expect(cap.body()).toBe('device page');
  });

  it("answers a client pair's GET with the not-found page when no device plane is composed", async () => {
    await pairing.startClientPair({ client: 'cli', peer: '127.0.0.1' });
    const handler = createPairingHttpHandler({ pairing });
    const cap = makeRes();
    handler(makeReq({ method: 'GET', url: '/pair/135790' }), cap.res);
    await cap.done;
    expect(cap.status()).toBe(404);
    expect(cap.body()).toContain('Pairing not found');
  });

  it('the admin confirm against a client pair reads as consumed — the code alone never yields a secret', async () => {
    const started = await pairing.startClientPair({ client: 'desktop', peer: '127.0.0.1' });
    const handler = createPairingHttpHandler({ pairing, clientPairView: () => undefined });
    const cap = makeRes();
    handler(makeReq({ method: 'POST', url: '/pair/135790/confirm', headers: JSON_HEADERS, body: '{}' }), cap.res);
    await cap.done;
    expect(cap.status()).toBe(410);
    expect(JSON.parse(cap.body())).toEqual({ ok: false, reason: 'consumed' });
    expect(cap.body()).not.toContain(started.pollToken);
    expect(await listDaemonAuthTokens()).toHaveLength(0);
  });

  it("keeps the admin pair's own confirm page — the seam never sees it", async () => {
    pairing.startPair({ deviceLabel: 'admin pair' });
    const seen: string[] = [];
    const handler = createPairingHttpHandler({ pairing, clientPairView: (pair) => seen.push(pair.code) });
    const cap = makeRes();
    handler(makeReq({ method: 'GET', url: '/pair/135790' }), cap.res);
    await cap.done;
    expect(seen).toEqual([]);
    expect(cap.status()).toBe(200);
    expect(cap.body()).toContain('Confirm pairing with this device');
  });
});
