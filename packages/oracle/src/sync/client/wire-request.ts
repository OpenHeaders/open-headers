 ...
 * of its type.
 *
 * Host-neutral (the Execution Place plan lifted it here from the
 * extension's service worker): every host that joins backends through
 * the connection manager — the extension SW, the desktop app's main
 * process — rides the same correlation. The extension keeps its
 * `background/ws-request` path as a re-export.
 */
