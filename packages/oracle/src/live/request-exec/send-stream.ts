/**
 * Live plumbing for interactive sends — host-neutral: the active-send
 * registry behind the `abortRequestSend` RPC (Stop button) and the
 * flush-batched `requestStreamEvent` frame emitter behind the response
 * panel's live tail. Both the browser SW and the node hosts run this
 * exact code; only the injected broadcast function differs (chrome
 * runtime messages / `webContents.send` / daemon WS frames).
 *
...
import type { RequestStreamEventWire, RequestStreamHeadWire } from '@openheaders/core/bridge';
...
export function createStreamEmitter(sendId: string, emit: (event: RequestStreamEventWire) => void): StreamEmitter {
  let seq = 0;
  let chunkFrames = 0;
  let settled = false;
  ...
  return {
    head(head) {
      if (settled) return;
      emit({ sendId, seq: seq++, kind: 'head', head });
    },
