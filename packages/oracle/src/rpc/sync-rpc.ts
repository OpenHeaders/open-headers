...
const SYNC_SNAPSHOT_DISPATCH: Record<string, (workspaceId?: string) => { entries: unknown[] }> = {
  ...
  'oh.sync.snapshotMqttRequests': (ws) => ({ entries: snapshotMqttRequestPostStates(ws) }),
  'oh.sync.snapshotGraphqlRequests': (ws) => ({ entries: snapshotGraphqlRequestPostStates(ws) }),
  ...
  'oh.sync.snapshotMqttResponseExamples': (ws) => ({ entries: snapshotMqttResponseExamplePostStates(ws) }),
